        ...
        <section id="main-content">
			...
			<small>五个哈哈团队</small>
                    ...
                    <div class="right-menu pull-right">
                        ...
                        <nav class="c_breadcrumbs">
                            <a href="" class="btn btn-green btn-flat btn-add-new-user" data-toggle="modal" data-target="#modal-pull-right-add"><span class="icon-user-follow"></span> 添加队伍</a>
                        </nav>
                    </div>
                ...
                <!--======== Page Title and Breadcrumbs End ========-->
						<jsp:useBean id="teamPage" class="com.zufe.bean.TeamPage"></jsp:useBean>
						<!-- 为javabean对象属性指定每页显示的信息数量 -->
						<jsp:setProperty property="pageSize" value="25" name="teamPage" />
						<jsp:setProperty property="pageNo" param="pageNo" name="teamPage" />
                <!--======== Table Content Start ========-->
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="c_panel">
                            ...
                            <div class="c_title">
                                ...
                                <!-- 页数信息 -->
                                <div align="center">
							<a href="manage/team_list.jsp?pageNo=1"><i  class="glyphicon glyphicon-step-backward" style="font-size: 15px;"></i></a>&nbsp;
							<%
								if (teamPage.isHasPreviousPage()) {
							%>
							<a
								href="manage/team_list.jsp?pageNo=<%=teamPage.getPageNo() - 1%>"><i  class="glyphicon glyphicon-chevron-left" style="font-size: 15px;"></i></a>&nbsp;
							<%
								}
							%>
							<%
								if (teamPage.isHasNextPage()) {
							%>
							<a
								href="manage/team_list.jsp?pageNo=<%=teamPage.getPageNo() + 1%>"><i  class="glyphicon glyphicon-chevron-right" style="font-size: 15px;"></i></a>&nbsp;
							<%
								}
							%>
							<a
							href="manage/team_list.jsp?pageNo=<%=teamPage.getTotalPages()%>"><i  class="glyphicon glyphicon-step-forward" style="font-size: 15px;"></i></a>&nbsp;
						当前是第<jsp:getProperty property="pageNo" name="teamPage" />页，总共<jsp:getProperty
							property="totalPages" name="teamPage" />页
					</div>
                            ...
                            <div class="c_content">
								...
								<th data-priority="3">省份</th>
								<th data-priority="3">队长</th>
								<th data-priority="3">人数</th>
								...
								list = teamPage.getPageData();
								if (list != null)
									for (Team t : list) {
							%>
						<tbody>
							<tr>
								<th> <span class="co-name"><%=t.getTeamId()%></span></th>
									<td><%=t.getTeamName()%></td>
									<td><%=t.getTeamProvince()%></td>
									<%if(t.getTeamLeader()==null){ %>
									<td>暂无</td>
									<%}else{ %>
									<td><%=userDao.getUserById(t.getTeamLeader())
							.getUserName()%></td>
									<%} %>
									<td><%=t.getTeamMemberNum()%></td>

									
								
								<td class="text-center"><a
									href="manage/team_info.jsp?id=<%=t.getTeamId()%>"><img src="img/info.png" style="width:20px;height:20px;"></a>
									
									<a
									class="delete btn btn-sm btn-danger btn-raised rippler rippler-inverse"
									href="<%=path%>/UserDeleteServlet?id=<%=t.getTeamId()%>&teamId=<%=t.getTeamId()%>"><i
